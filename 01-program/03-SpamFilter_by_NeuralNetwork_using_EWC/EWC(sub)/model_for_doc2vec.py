...
class Model:
    ...
    def compute_fisher(self, doc2vec_model, sess, num_samples=200,
                       plot_diffs=False, disp_freq=10):
        ...
        sum_validation = []
        sum_validation.extend(list(range(5000, 5100)))  # [5000,5001,...,5099]
        ...
        for i in range(num_samples):  # num_samples=200
            ...
            ders = sess.run(
                tf.gradients(tf.log(probs[0, class_ind]), self.var_list),
                feed_dict={
            ...
            if (plot_diffs):  # plot処理
                if i % disp_freq == 0 and i > 0:  # disp_freq=10
                    ...
                    for v in range(len(self.F_accum)):
                        # np.absolute:絶対値を求める。
                        F_diff += np.sum(
                            np.absolute(self.F_accum[v] / (i + 1) - F_prev[v]))
    ...
    def update_ewc_loss(self, lam):
        ...
        for v in range(len(self.var_list)):
            fisher = (lam / 2) * tf.reduce_sum(
                tf.multiply(self.F_accum[v].astype(np.float32),
                    tf.square(self.var_list[v] - self.star_vars[v])))

            self.ewc_loss += fisher

            # with tf.Session() as sess:  #    fisher = sess.run(fisher)

            # parameters_fisher[v, v] = fisher

        # df = pd.DataFrame(data=parameters_fisher, index=['W1', 'b1', 'W2',
        # 'b2'], columns=['W1', 'b1', 'W2', 'b2'])
        # plt.figure()
        # sns.heatmap(df, cmap='Blues')
        # plt.show()

        self.train_step = tf.train.GradientDescentOptimizer(0.001).minimize(
            self.ewc_loss)  # 学習率0.1として、ewcで求めた損失関数値を最小にするようにパラメータを学習する。
