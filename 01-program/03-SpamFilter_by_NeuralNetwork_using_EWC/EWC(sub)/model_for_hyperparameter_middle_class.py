...
class Model:
    def __init__(self, neuron_num, learning_rate, middle_class_num, x, y_):

        middle_class_weight_and_bias_list = []
        relu_output_list = []
        ...
        W_in = weight_variable([in_dim, neuron_num])
        b_in = bias_variable([neuron_num])

        if middle_class_num >= 2:
            for _ in range(middle_class_num-1):
                middle_class_weight_and_bias_list.append(
                    weight_variable([neuron_num, neuron_num]))
                middle_class_weight_and_bias_list.append(
                    bias_variable([neuron_num]))

        W_out = weight_variable([neuron_num, out_dim])
        b_out = bias_variable([out_dim])

        # ReLU関数：活性化関数の一つで、入力した値が0以下のときに0、0より大きい場合はそのままの値を出力する関数。
        h_in = tf.nn.relu(tf.matmul(x, W_in) + b_in)  # hidden layer
        relu_output_list.append(h_in)

        if middle_class_num >= 2:
            for middle_class_idx in range(middle_class_num - 1):
                h = tf.nn.relu(tf.matmul(
                    relu_output_list[-1], middle_class_weight_and_bias_list[middle_class_idx*2]) + middle_class_weight_and_bias_list[middle_class_idx*2 + 1])
                relu_output_list.append(h)

        self.y = tf.matmul(relu_output_list[-1], W_out) + b_out  # output layer

        self.var_list = [W_in, b_in] + \
            middle_class_weight_and_bias_list + [W_out, b_out]
