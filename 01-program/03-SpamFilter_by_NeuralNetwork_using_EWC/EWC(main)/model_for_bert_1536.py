...
class Model:
    def __init__(self, x, y_):
        ...
        in_dim = 1536  # doc2vecを用いて得られたベクトルの次元数
        out_dim = 2  # スパムメールと正規メールの2クラス

        self.x = x  # input placeholder (x:[None,300])

        # 入力層と中間層の間のパラメータ
        W1 = weight_variable([in_dim, 1024])
        ...
        h2 = tf.nn.relu(tf.matmul(h1, W2) + b2)  # hidden 1 layer
    ...
    def compute_fisher(self, bert_model_dict, sess, num_samples=400, plot_diffs=False, disp_freq=10):
        """
        フィッシャー情報量の計算
        """
        bert_model = bert_model_dict['2005']
        bert_model.extend(bert_model_dict['2006'])
        ...
        for i in range(num_samples):  # num_samples=400
            ...
            ders = sess.run(tf.gradients(tf.log(probs[0, class_ind]), self.var_list), feed_dict={
                            self.x: np.array(bert_model[im_ind]).reshape(1, 1536)})
            ...
            if (plot_diffs):  # フィッシャー情報量のプロット処理
                if i % disp_freq == 0 and i > 0:  # disp_freq=10
                    ...
                    plt.plot(range(disp_freq + 1, i + 2, disp_freq),
                             mean_diffs, marker='.')
    ...
    def set_vanilla_loss(self):
        ...
        self.train_step = tf.compat.v1.train.GradientDescentOptimizer(
            0.000007).minimize(self.cross_entropy)  # 元々0.000007
    ...
    def update_ewc_loss(self, lam):
        ...
        self.train_step = tf.train.GradientDescentOptimizer(
            0.000007).minimize(self.ewc_loss)  # 元々0.000007
    ...
    def multiply_weight(self, sess):  # 一回学習するごとに重みに0.99を乗算する
        ...
        for i, var in enumerate(self.var_list):
            if i % 2 == 0:
                ...
                # 0.99999(learning rateは0.009)が良さげだった
                const = tf.constant(0.9997)
