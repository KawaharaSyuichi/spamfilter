...
BERTの1536次元(ヘッダと本文を連結)でMVG(とCMP)を実行
"""
import sys
import csv
import copy
...
from model_for_bert_1536 import Model
...
def MVG(model, mode, mail_bert, mail_class, bert_dict, num_iter, disp_freq, sess, lams, same_flag=False):
    train_start_idx = 0
    bert_2005 = copy.copy(bert_dict['2005'])
    bert_2006 = copy.copy(bert_dict['2006'])
    bert_2007 = copy.copy(bert_dict['2007'])
    ...
    for iteration in range(num_iter):
        ...
        # 学習開始
        model.train_step.run(feed_dict={mail_bert: np.array(
            train_batch[0]), mail_class: np.array(train_batch[1])})

        # disp_freq(=20)回学習するごとに、テスト用メールに対する識別率を求める
        if iteration % disp_freq == 0:
            # 2005年のテスト用データに対する識別率を算出
            test_batch_2005 = make_test_batch(bert_2005)

            feed_dict_test_2005 = {mail_bert: np.array(
            ...
            train_batch_2005 = make_train_batch(bert_2005)

            feed_dict_train_2005 = {mail_bert: np.array(
            ...
            # 2006年のテスト用データに対する識別率を算出
            test_batch_2006 = make_test_batch(bert_2006)

            feed_dict_test_2006 = {mail_bert: np.array(
                test_batch_2006[0]), mail_class: np.array(test_batch_2006[1])}
            ...
            # 2006年の学習用データに対する識別率を算出
            train_batch_2006 = make_train_batch(bert_2006)

            feed_dict_train_2006 = {mail_bert: np.array(
                train_batch_2006[0]), mail_class: np.array(train_batch_2006[1])}
            ...
            if iteration < 800:  # iterationが800未満の場合、2007年の分はNoneで埋める
                ...
                # 2007年のテスト用データに対する識別率を算出
                test_batch_2007 = make_test_batch(bert_2007)

                feed_dict_test_2007 = {mail_bert: np.array(
                    test_batch_2007[0]), mail_class: np.array(test_batch_2007[1])}
                ...
                # 2007年の学習用データに対する識別率を算出
                train_batch_2007 = make_train_batch(bert_2007)

                feed_dict_train_2007 = {mail_bert: np.array(
                    train_batch_2007[0]), mail_class: np.array(train_batch_2007[1])}
    ...
    if same_flag == False:  # 学習用とテスト用の識別率を別々にプロット
        ...
        plt.ylim(bottom=y_min, top=1.01)
        if mode == 'NOTEWC':
...
def main():
    ...
    model_info_orderdict = OrderedDict({
        '2005': common_path + '2005_2000_mails_BERT.csv',
        '2006': common_path + '2006_2000_mails_BERT.csv',
        '2007': common_path + '2007_2000_mails_BERT.csv'
    })
    ...
    for doc2vec_label, file_path in model_info_orderdict.items():
        with open(file_path, "r") as f:
            reader = csv.reader(f)
            model_bert_dict[doc2vec_label] = [
                [float(v) for v in row] for row in reader]

    sess = tf.compat.v1.InteractiveSession()

    mail_bert = tf.compat.v1.placeholder(
        tf.float32, shape=[None, 1536])  # bertの場合1536
    mail_class = tf.compat.v1.placeholder(
        tf.float32, shape=[None, 2])  # 教師ラベルの種類　ham:[1,0],spam:[0,1]

    # ニューラルネットワークモデルの構築
    model = Model(mail_bert, mail_class)
